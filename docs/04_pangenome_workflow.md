...
| KOfam / KEGG | (bundled with anvi'o) | Functional annotation of predicted genes |
| Python | 3.10 | Required by anvi'o v9 |
| conda | — | Environment management |

Platform: WSL2 / Linux

### Installing anvi'o v9 from scratch

The installation follows the [official anvi'o v9 install guide for Windows/WSL](https://anvio.org/install/). The steps below are the exact commands used in this project.
...
The `fastani` step is optional — the official guide notes that it is useful but not required for core functionality.
...
If `pip install` fails with a `gcc` error:
...
This is a known issue mentioned in the official install guide — WSL may not have `gcc` pre-installed.
...
# Pangenomics-specific test (recommended before running the real analysis)
anvi-self-test --suite pangenomics --no-interactive
```

The official guide recommends running `--suite pangenomics` if you plan to use the pangenome workflow. Under WSL, all interactive anvi'o commands (`anvi-display-pan`, `anvi-interactive`, etc.) require `-I localhost` because WSL cannot auto-launch a browser.

### Working directory setup

```bash
mkdir -p ~/projects/anvio_isol153/{fasta,contigs_db,results,logs}
cd ~/projects/anvio_isol153
```

A dedicated project directory keeps input genomes, databases, and outputs organized in one place, separate from the main `isolates/` directory.
